          ...
          <table className="w-full min-w-[760px] text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-800">
              {vouchers.map((voucher) => (
                <tr key={voucher.id} className="text-slate-300">
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <form onSubmit={saveVoucher} className="max-h-[90vh] w-full max-w-lg space-y-4 overflow-y-auto rounded-2xl border border-slate-700 bg-slate-900 p-5 text-white shadow-2xl sm:p-6">
            ...
            <div><label htmlFor="voucher-code" className="mb-1.5 block text-xs font-bold text-slate-300">Mã voucher</label><input id="voucher-code" required maxLength={32} value={form.code} onChange={(e) => setForm((current) => ({ ...current, code: e.target.value.toUpperCase() }))} placeholder="VD: THUECAM10" className={`${inputClass} font-mono font-black uppercase`} /></div>
            <div className="grid grid-cols-2 gap-3">
              <div><label htmlFor="voucher-type" className="mb-1.5 block text-xs font-bold text-slate-300">Loại ưu đãi</label><select id="voucher-type" value={form.discount_type} onChange={(e) => setForm((current) => ({ ...current, discount_type: e.target.value as VoucherForm['discount_type'] }))} className={inputClass}><option value="PERCENT">Phần trăm (%)</option><option value="FIXED">Số tiền (VNĐ)</option></select></div>
              <div><label htmlFor="voucher-value" className="mb-1.5 block text-xs font-bold text-slate-300">Giá trị giảm</label><input id="voucher-value" type="number" min="1" max={form.discount_type === 'PERCENT' ? 100 : 100000000} required value={form.discount_value} onChange={(e) => setForm((current) => ({ ...current, discount_value: e.target.value }))} className={inputClass} /></div>
            </div>
            <div><label htmlFor="voucher-limit" className="mb-1.5 block text-xs font-bold text-slate-300">Giới hạn lượt dùng <span className="font-normal text-slate-500">(để trống là không giới hạn)</span></label><input id="voucher-limit" type="number" min="1" max="1000000" value={form.max_uses} onChange={(e) => setForm((current) => ({ ...current, max_uses: e.target.value }))} className={inputClass} /></div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div><label htmlFor="voucher-start" className="mb-1.5 block text-xs font-bold text-slate-300">Bắt đầu từ</label><input id="voucher-start" type="datetime-local" required value={form.starts_at} onChange={(e) => setForm((current) => ({ ...current, starts_at: e.target.value }))} className={inputClass} /></div>
              <div><label htmlFor="voucher-end" className="mb-1.5 block text-xs font-bold text-slate-300">Hết hạn <span className="font-normal text-slate-500">(tùy chọn)</span></label><input id="voucher-end" type="datetime-local" value={form.expires_at} onChange={(e) => setForm((current) => ({ ...current, expires_at: e.target.value }))} className={inputClass} /></div>
            </div>
            <label className="flex items-center gap-2 text-xs font-bold text-slate-300"><input type="checkbox" checked={form.is_active} onChange={(e) => setForm((current) => ({ ...current, is_active: e.target.checked }))} className="size-4 accent-sky-500" />Cho phép sử dụng</label>
            {errorMessage && <p role="alert" className="text-xs font-bold text-rose-300">{errorMessage}</p>}
            <div className="flex justify-end gap-2 border-t border-slate-800 pt-4"><button type="button" onClick={() => setIsFormOpen(false)} className="rounded-xl bg-slate-800 px-4 py-2.5 text-xs font-bold text-slate-300">Hủy</button><SafeButton type="submit" disabled={isSaving} loadingText="Đang lưu..." className="inline-flex items-center gap-2 rounded-xl bg-sky-500 px-4 py-2.5 text-xs font-black text-slate-950 hover:bg-sky-400"><Save className="size-4" />Lưu voucher</SafeButton></div>
          </form>
        </div>
      )}
    </div>
