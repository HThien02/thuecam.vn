...
export default async function HourlyRentalPage() {
  ...
  return (
    <main className="min-h-screen bg-[#fffaf7] text-slate-900">
      <section className="border-b border-[#d9eee8] bg-[#f2fbf7]">
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-7 flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-[#ff6b9a]">Bảng giá theo giờ</p><h2 className="mt-2 text-3xl font-black">Chọn máy bạn cần</h2></div><span className="hidden text-sm text-slate-500 sm:block">{products.length} thiết bị</span></div>
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {products.map((product) => (
            <article key={product.id} className="overflow-hidden rounded-[2rem] border border-[#f3dce5] bg-white shadow-[0_16px_35px_-26px_rgba(31,41,55,.5)] transition hover:-translate-y-1 hover:shadow-xl">
              <div className="relative bg-[#f8f7f5] p-4"><span className={`absolute left-6 top-6 rounded-full px-3 py-1 text-xs font-black ${product.inventory_count > 0 ? 'bg-[#d9f4e9] text-[#168267]' : 'bg-amber-100 text-amber-900'}`}>{product.inventory_count > 0 ? 'Còn máy' : 'Full lịch thuê'}</span><img src={product.primary_image} alt={product.name} className="h-56 w-full rounded-[1.5rem] object-contain" /></div>
              <div className="flex flex-col gap-4 p-5"><div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">{product.category?.name ?? 'Thiết bị quay'}</p><h3 className="mt-1 text-lg font-black">{product.name}</h3></div><div className="flex items-end justify-between gap-3"><div><p className="text-xs text-slate-500">Giá 6 tiếng từ</p><p className="text-2xl font-black text-[#168267]">{formatPrice(Math.max(200000, product.rental_price_per_day * 0.75))}<span className="text-sm">/6h</span></p></div><Link href={`/dat-thue?product=${product.slug}&duration=hourly`} className="inline-flex items-center gap-1 rounded-full bg-[#ff6b9a] px-4 py-2.5 text-xs font-black text-white shadow-[0_10px_20px_-12px_#ff6b9a] hover:bg-[#ed5688]">{product.inventory_count > 0 ? 'Đặt thuê ngay' : 'Xem lịch'} <ArrowRight className="size-3.5" /></Link></div></div>
            </article>
          ))}
        </div>
